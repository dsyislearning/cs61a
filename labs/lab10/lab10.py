...
def insert(link, value, index):
    ...
    p = link
    i = 0
    while p.rest is not Link.empty and i != index:
        p = p.rest
        i += 1
    if p.rest is not Link.empty and i == index:
        p.rest = Link(p.first, p.rest)
        p.first = value
    elif p.rest is Link.empty and i == index:
        p.rest = Link(p.first, Link.empty)
        p.first = value
...
def reverse_other(t):
    ...
    """
    def reverse_helper(t, d):
        if d % 2 == 0:
            labels = list(map(lambda x: x.label, t.branches))[::-1]
            for i in range(len(labels)):
                t.branches[i].label = labels[i]
        for b in t.branches:
            reverse_helper(b, d + 1)
