...
def count_cond(condition):
    ...
    """def count(N):
        res, k = 0, 1
        while k <= N:
            res, k = res + 1 if condition(N, k) else res, k + 1
        return res
    return count"""
...
def multiple(a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    f = lambda a, b: a if b == 0 else f(b, a % b) # 递归辗转相除发求最大公因数
    return a * b // f(a, b)
...
def cycle(f1, f2, f3):
    ...
    def f(n):
        if n == 0:
            ...
            return lambda x: f3(f2(f1(f(n - 3)(x))))
        elif n % 3 == 1:
            return lambda x: f1(f(n - 1)(x))
        elif n % 3 == 2:
            return lambda x: f2(f1(f(n - 1)(x)))
    return f
