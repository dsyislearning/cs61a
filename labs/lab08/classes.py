...
class TutorCard(Card):
    ...
    def power(self, opponent_card): # override
        return -float('inf')
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        for i in range(len(player.hand)):
            if player.hand[i].power(opponent_card) > player.hand[best_index].power(opponent_card):
                best_index = i
        discarded_card = player.hand.pop(best_index)
        self.attack += discarded_card.attack
        self.defense += discarded_card.defense
        best_card = discarded_card
        # You should add your implementation above this.
        if best_card:
