...
def pick(paragraphs, select, k):
    ...
    if k in range(len(selected_paragraphs)):
...
def about(subject):
    ...
    def select(paragraph):
        subject_list = [remove_punctuation(lower(s)) for s in subject]
        paragraph_word_list = split(remove_punctuation(lower(paragraph)))
        for s in subject_list:
            if s in paragraph_word_list:
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if typed_len >= source_len:
        if typed_len == 0:
            ...
            correct_words = [typed_words[i] for i in range(source_len) if typed_words[i] == source_words[i]]
            return len(correct_words) / typed_len * 100.0
    else:
        if typed_len == 0:
            return 0.0
        else:
            correct_words = [typed_words[i] for i in range(typed_len) if typed_words[i] == source_words[i]]
            return len(correct_words) / typed_len * 100.0
    # END PROBLEM 3
