...
def balanced(m):
    ...
    if is_mobile(right_end):
        if not balanced(right_end):
            return False
    return total_weight(left_end) * length(left_arm) == total_weight(right_end) * length(right_arm)
...
def mul_interval(x, y):
    ...
    value in y."""
    lower_bound_x = lower_bound(x)
    lower_bound_y = lower_bound(y)
    upper_bound_x = upper_bound(x)
    upper_bound_y = upper_bound(y)
    p1 = lower_bound_x * lower_bound_y
    p2 = lower_bound_x * upper_bound_y
    p3 = upper_bound_x * lower_bound_y
...
def sub_interval(x, y):
    ...
    upper_bound_x = upper_bound(x)
    upper_bound_y = upper_bound(y)
    d1 = lower_bound_x - lower_bound_y
    d2 = lower_bound_x - upper_bound_y
    d3 = upper_bound_x - lower_bound_y
    d4 = upper_bound_x - upper_bound_y
